...
class FFNN():
	...
	# Find the accuracy
	def my_modelPerformance(self, x_test, y_test):
		predictions = []
		y_true = []
		y_pred = []
		losses = []
	...
	def do_stochastic_gradient_descent(self, x_train, y_train, x_val, y_val):
		...
		for epoch in range(self.epochs):
			...
			# Validation Accuracy
			train_acc, train_loss, y_true, y_pred = self.my_modelPerformance(x_train, y_train)
			val_acc, val_loss, y_true, y_pred = self.my_modelPerformance(x_val, y_val)
